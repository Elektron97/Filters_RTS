...
#define PRIO_EXPLOSION		3		// try to put the same priority
#define PER_EXPLOSION		25
#define DREL_EXPLOSION		25
...
// EXPLOSION CONSTANTS
//----------------------------------------------------------------------------
#define N_DEBRIS 				100 	// total number of debris BITMAP
...
#define TSCALE_DEBRIS			2		// time scale factor for debris motion
...
int 	skeet_hit = 0;		// flag that triggers explosion task
int 	end = 0;			// end flag
int 	shot_f = 0; 		// flag is set to one if user clicks space	
int 	debris_small_gen_glob; // informa explosion draw about the number of debris
//---------------------------------------------------------------------------
// GLOBAL BITMAPS
//---------------------------------------------------------------------------
BITMAP* aim_screen;
...
// GLOBAL SEMAPHORES
//---------------------------------------------------------------------------
pthread_mutex_t			mux_skeet_state, mux_skeet_alive, mux_skeet_awake, mux_skeet_hit,
						mux_aim_coordinate, mux_shot, mux_debris_small; 
						// define 5 mutex
...
int i;
	...
	//----------------------------------------------------------------------------
	// INITIALIZE EXPLOSION DEBRIS
	//----------------------------------------------------------------------------
	// Definition of small debris geometry for each type:
	// each row of the matrix is a different polygon
	...
	pthread_mutex_init(&mux_skeet_hit, &matt);
...
int r_sq;				// square of the radius
	...
	if (dist_shot_sq <= r_sq)		// skeet hit!
	{
		...
		pthread_mutex_lock(&mux_skeet_hit);			// wait on the mutex for skeet hit flag
		skeet_hit = 1;
		pthread_mutex_unlock(&mux_skeet_hit);		// signal on the mutex for skeet hit flag
	}
	else
		printf("miss\n");			// skeet missed
}
...
int shot_f_local;	// to memorize global variable
	...
	// Before to sleep start the initialization of explosion_task
...
//--------------------------------------------------------------------------
// Preparing the geometry and velocities of debris after user presses C_KEY

int r;						// radius of the skeet
int col;					// color of the skeet
int a = IDX_SKEET_TASK; 	// index of the task the will explode
...
// scriviamo le coordinate del quadrato BLU che genera gli skeet (frame della explosion BITMAP)
...
int	i = 0; 	// debris index 
	...
	while(deb_BP_xl < rect_xr) 		// top side of rect
	{
		...
		polygon(debris_bp[i], 5, deb_small[type_extract].values, col);	// draw the type of bride extracted randomly into a BITMAP
		...
		// printf("i = %d => xc: %f yc: %f", i, deb_small[i].xc, deb_small[i].yc);
...
//------------------------------------------------------------------------------
	...
	debris_small_gen_glob = i;	// global variable to inform draw_explosion about the number of small debris
	printf("%d\n", debris_small_gen_glob);

	float dt;
	dt = TSCALE_DEBRIS * (float) get_task_period(arg) / 1000;

	printf("%f\n", dt);
	int skeet_hit_local; 	// local variable that memorize skeet_hit flag

	int idx_expl; 		// index for explosion_task
	idx_expl = get_task_index(arg);
	set_activation(idx_expl);

	// the explosion task computes physich of the debris motion
	while(!end)
	{	
		pthread_mutex_lock(&mux_skeet_hit);		// wait on the mutex for skeet alive flag		
		skeet_hit_local = skeet_hit;
		pthread_mutex_unlock(&mux_skeet_hit);	// wait on the mutex for skeet alive flag
		// if skeet is hit computes physic
		if(skeet_hit_local)
		{	
			pthread_mutex_lock(&mux_debris_small); 		// wait on the mutex for small debris position

			for(i = 0; i < debris_small_gen_glob; i++)
			{
			...
			pthread_mutex_unlock(&mux_debris_small); 	// signal on the mutex for small debris position
		}
...
//----------------------------------------------------------------------------
// DRAW_EXPLOSION: draw brides of the explosion
//----------------------------------------------------------------------------
void draw_explosion(int i, BITMAP* world_copy, BITMAP* explosion_copy)
{
int xc, yc; 	// last coordinates of the skeet respect to world_copy frame
// x e y sono gli ultimi valori del centro dello skeet
int k;

		xc = skeet[i].x;			// transform coordinates of skeet into left top point of skeet_image BITMAP
		yc = -(skeet[i].y - YBWORLD + YTWORLD);

		// circlefill(explosion_copy, 3*r, 3*r, r, GREEN); //cerchio di raggio r
		// rect(explosion_copy, rect_xl, rect_yt, rect_xr, rect_yb, LIGHT_BLUE); // quadrato di lato r/3

		pthread_mutex_lock(&mux_debris_small); 		// wait on the mutex for small debris position

		for(k = 0; k < debris_small_gen_glob; k++){
			draw_sprite(explosion_copy, debris_bp[k], deb_small[k].xl, deb_small[k].yt);
			printf("%d\n", k);
		}
		...
		blit(explosion_copy, world_copy, 0, 0, xc-W_EXPL_BITMAP/2, yc - W_EXPL_BITMAP/2,
			 explosion_copy->w, explosion_copy->w);	

		// circlefill(world_copy, xc, yc, 5, RED);
		//	CANCELLARE BITMAP NON USATE!!
		// LA DIMENSIONE DI EXPLOSION COPY è costante per ora
}
//----------------------------------------------------------------------------
// DISPLAY_TASK (TASK): 
//----------------------------------------------------------------------------
...
int skeet_hit_local; 		// copy global variable in a local variable for semaphore
char s[30];				// string for monitor deadline messed
	...
	explosion_copy = create_bitmap(W_EXPL_BITMAP, W_EXPL_BITMAP);		// create a bitmap with the resolution proportional to skeet[i].r
	...
	while(!end)
	{
		...
		// if skeet is hit draw explosion
		pthread_mutex_lock(&mux_skeet_hit);		// wait on the mutex for skeet alive flag		
		skeet_hit_local = skeet_hit;
		pthread_mutex_unlock(&mux_skeet_hit);		// wait on the mutex for skeet alive flag
		if (skeet_hit_local)
	...
	{	
		...
		if (keypressed())	// returns true if there are characters in keyboard buffer, else false (it does not block the program execution)
		{
			...
			switch (scan)
			{
				...
				case KEY_C:
					// check if a skeet is alive (flying)
					pthread_mutex_lock(&mux_skeet_alive);		// wait on the mutex for skeet alive flag
					skeet_alive_local = skeet_alive;
					pthread_mutex_unlock(&mux_skeet_alive);		// signal on the mutex for skeet alive flag
					if (skeet_alive_local == 0){				// if no skeet are in the window
						// task is alive but it is sleeping for a random time;
						pthread_mutex_lock(&mux_skeet_alive);		// wait on the mutex for skeet alive flag
						skeet_alive++;
						pthread_mutex_unlock(&mux_skeet_alive);		// signal on the mutex for skeet alive flag
						ammo = 2;									// reload the rifle
						// from this time is possible to shot and lose ammo even if no task is alive
